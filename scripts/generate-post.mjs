...
## 記事の要件
1. 1500〜2500文字の本文(frontmatter除く)
2. 信頼性重視: 具体的な地名・数字・固有名詞を入れる(架空ではなく実在の地名・施設を使う)
3. 実用的: 読者が次の旅に活かせる情報を含める
4. 構成: 導入(リード文) → H2見出しで2〜4セクション → 各セクションに具体例 → 最後にまとめ
5. 過度な装飾や絵文字の乱用は避け、ジャーナリスティックで親しみやすい文体で書く
6. 「私は」「筆者は」のような一人称体験談は使わない(検証不能なので)
7. 観光地の情報は一般的に知られている事実のみ使う(料金や時刻など変動する情報は避けるか「2025年時点」等の注釈を入れる)
